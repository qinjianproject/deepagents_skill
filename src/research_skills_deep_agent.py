...
def make_backend(runtime):
    ...
    return CompositeBackend(
        # 修改这里：为默认后端指定根目录，避免直接访问系统根目录 /
        default=FilesystemBackend(root_dir="./agent_storage", virtual_mode=False),  
        routes={
...
USER_SKILLS_DIR = "/home/huawei/my_project/deepagents-demo/agent/skills"
...
#     try:
#         # 使用 stream 模式迭代获取更新
#         for chunk in agent.stream({"messages": [{"role": "user", "content": stock_query}]}, config=config, stream_mode="values"):
#             if "messages" in chunk:
#                 last_msg = chunk["messages"][-1]
                
#                 # 1. 捕获并打印 AI 的回复（包含普通文本）
...
#                     print(f"✅ [工具返回] 详细信息:")
#                     # 如果返回的是长文本或 JSON，这里会完整打印
#                     print(f"{last_msg.content[:2000]}...") # 限制一下长度防止刷屏，可根据需要调整
#                     print("-" * 30)
